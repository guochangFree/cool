 ...
 */
package com.go.cool.common.utils;


import com.go.cool.common.Constants;
import com.go.cool.common.extention.ExtensionLoader;
...
public class ConfigUtils {
    ...
    public static Properties getProperties() {
        if (PROPERTIES == null) {
            synchronized (ConfigUtils.class) {
                if (PROPERTIES == null) {
                    String path = System.getProperty(Constants.COOL_PROPERTIES_KEY);
                    if (path == null || path.length() == 0) {
                        path = System.getenv(Constants.COOL_PROPERTIES_KEY);
                        if (path == null || path.length() == 0) {
                            path = Constants.DEFAULT_COOL_PROPERTIES;
                        }
                    }
     ...
     *
     * @param fileName       properties file name. for example: <code>cool.properties</code>, <code>METE-INF/conf/foo.properties</code>
    ...
    public static Properties loadProperties(String fileName, boolean allowMultiFile, boolean optional) {
        ...
        if (!allowMultiFile) {
            if (list.size() > 1) {
                String errMsg = String.format("only 1 %s file is expected, but %d cool.properties files found on class path: %s",
